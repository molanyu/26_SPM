...
| US-14 | G6 | `violation` `reservation` `admin_portal` | `GET /admin/reservations` `GET /admin/reservations/records` `GET /admin/violations` `POST /admin/violations/users/{user_id}/manual-block` `POST /admin/violations/users/{user_id}/manual-block/release` | 管理员可按条件查询预约记录和违约记录；违约记录无筛选时显示全部且 HTML 错误不返回裸 JSON；违约记录页可展示指定用户的违约次数统计和统一预约限制状态；具备 `violation.manual_blocks.write` 权限的管理员可手动开启或解除指定用户无期限预约限制；预约创建前必须被 `reservation` 通过 `violation` 统一只读状态拦截 | 预约记录筛选测试、违约记录筛选测试、用户级违约次数统计测试、`violation.manual_blocks.write` 权限下手动预约限制开启/解除测试、缺少 `violation.manual_blocks.write` 的无权限测试、学生和管理员代预约限制拦截测试、违约页 HTML 错误测试 |
| US-15 | G4 | `reservation` | `POST /admin/reservations` `POST /admin/reservations/{reservation_id}/cancel` | 管理员可代预约和代取消 | 代理操作测试 |
| US-16 | G7 | `violation` `resource` `admin_portal` | `GET /admin/statistics/usage` `GET /admin/statistics` `GET /admin/violations` | 管理员可查看使用率和违约率统计；违约记录页的用户级违约次数统计只作为单用户查询辅助，不扩展为复杂统计大盘 | 统计查询测试、管理端统计页测试、违约页单用户统计展示测试 |
