...
- 创建预约前必须校验目标用户是否处于 `violation` 返回的统一预约限制状态。
- 统一预约限制状态包含自动违约累计惩罚和管理员手动预约限制。
- 被统一预约限制命中的用户不得创建新的预约。
- 管理员代预约不得绕过统一预约限制拦截，除非后续 spec 明确增加豁免规则。
- 统一预约限制状态由 `violation` 模块公开只读 service 计算，`reservation` 不直接读取 `violation` repository 或手动限制数据表。
- 指定时段座位占用查询归属 `reservation` 编排，基于状态为 `BOOKED` 或 `CHECKED_IN` 且时间区间重叠的预约记录判断。
- `resource` 只提供资源基础列表、可见性、开放时间与属性筛选，不负责计算预约占用。
- `project_blueprint` 已允许 `reservation -> violation` 依赖，但仅限公开只读 service；预约限制拦截只能消费该 service 的统一预约限制判断结果。
...
- `reservation` 可以在创建预约前读取 `violation` 的统一预约限制判断结果，但不得生成、修改或删除违约记录或手动限制记录
...
- `reservation` 需要依赖 `violation` 的公开只读 service 获取目标用户统一预约限制状态；该依赖已在 `project_blueprint` 中允许，且不得直接读取 `violation` repository 或手动限制数据表
...
- 目标用户是否处于自动违约惩罚或手动预约限制状态，由 `violation` 提供统一判断结果
...
- 代预约仍必须通过同样的时间、冲突、资源可用性和统一预约限制校验

### 4.2.1 统一预约限制拦截

- 学生自助预约必须校验当前学生是否处于 `violation` 返回的统一预约限制状态
- 管理员代预约必须校验目标学生是否处于 `violation` 返回的统一预约限制状态
- 统一预约限制状态可以来自自动违约累计惩罚，也可以来自管理员手动预约限制
- 处于统一预约限制状态时，创建预约必须被拒绝，并返回受控业务错误
- 预约限制拦截发生在写入预约前，且不得先创建预约再回滚
- 预约限制拦截不得影响历史预约查询、当前预约查询或取消已有预约
- 预约限制拦截不得在 `reservation` 内重复计算违约累计口径，不得读取 `violation` repository 或手动限制表，只能消费 `violation` 的公开只读结果
- 手动预约限制第一版为无期限直到管理员解除；`reservation` 不负责解除或到期处理
...
- 已注销或未启用资源的预约被拒绝
- 处于自动违约惩罚期的学生创建预约被拒绝
- 管理员代预约处于自动违约惩罚期的学生被拒绝
- 处于管理员手动预约限制状态的学生创建预约被拒绝
- 管理员代预约处于手动预约限制状态的学生被拒绝
- 同一座位冲突预约被拒绝
