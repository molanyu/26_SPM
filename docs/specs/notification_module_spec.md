...
- 为支撑本地手动验收，允许 `admin_portal` 通过本模块公开 service 提供最小通知日志查看和内部任务手动触发页面
- 手动触发入口不得替代调度系统，不引入复杂任务编排；仅允许触发 `RESERVATION_REMINDER`、`NO_SHOW_REMINDER`、`AUTO_CANCEL_NOTICE` 三类已有任务，并支持传入固定 `now`
...
- 管理端或脚本可查看通知日志并手动触发三类内部通知任务，用于解释未收到邮件或日志缺失的原因
- 关键通知测试通过
- 邮件模板内容测试通过
