...
以下规则用于修复手动验收中发现的“预约已经开始后仍显示取消预约”和“同一预约在当前/历史区域造成误解”的问题：

- 当前有效预约只有在状态为 `BOOKED` 且预约开始时间仍晚于当前时间时，才允许展示“取消预约”按钮。
- 如果预约已经开始但后端状态仍暂时为 `BOOKED`，学生端应显示“待签到/不可取消”等过渡状态，不得继续展示可取消操作。
- 预约状态文案必须区分 `BOOKED`、`CHECKED_IN`、`CANCELLED`、`EXPIRED`；其中 `EXPIRED` 面向学生展示为超时未签到/违约相关状态，不得展示为可取消预约。
- 我的预约页不得让同一 `reservation_id` 在当前有效区域和历史区域中被误认为两条独立预约；可过滤当前项，或明确标记为同一条记录。
...
- 助手页必须将后端结构化结果转换为面向学生的卡片、列表、空状态和错误状态，不得直接展示 `JSON.stringify(response.result)` 之类的 raw JSON
...
- 学生可完成动态码签到或二维码签到
- 学生可提交助手查询并获得受控结果
- 助手查询结果不展示程序员 JSON，空结果和失败结果都有清楚的用户态文案
