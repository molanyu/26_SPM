...
- 违约记录页面的手动预约限制开启和解除按钮由 `violation.manual_blocks.write` 权限控制；无该权限时不得展示写操作入口
...
- 页面不得承担后台自动流程推进动作；通知提醒、超时释放、动态码轮换等时间驱动流程必须由后台任务或对应业务 service 自动完成
...
- 页面不得在浏览器侧重写复杂筛选逻辑
- 违约记录页面展示的用户级违约次数统计必须来自 `violation` 公开 service/API，不得在页面或浏览器侧累计表格行
- 违约记录页面的手动预约限制按钮只提交目标用户和原因，最终状态以 `violation` 返回结果为准
...
- 违约记录页面调用 `violation` 公开 service 查询记录、用户级统计、统一预约限制状态，并开启或解除手动预约限制
- 违约记录页面开启和解除手动预约限制时必须走受 `violation.manual_blocks.write` 保护的 `violation` 公开 service/API
- 违约记录页面不得直接操作 `violation` repository、`ViolationRecord` 或 `UserReservationBlock`
...
- `/admin/violations` 用于查询违约记录、查看指定用户的违约次数统计和当前预约限制状态，并提供手动开启或解除该用户预约限制的表单动作
- 违约记录页面的手动预约限制第一版只支持单个用户、无期限直到解除，不提供批量限制或自动到期
- 违约记录页面不得提供人工申诉、信用分、多级惩罚配置或复杂统计大盘
- `/admin/notifications` 只用于查看和筛选通知日志，不提供手动触发通知任务、复杂调度或 SMTP 配置管理
...
- 违约记录页面在指定单个用户时展示该用户违约次数统计和统一预约限制状态
- 违约记录页面可提交手动开启预约限制并展示受控成功或错误信息
- 违约记录页面可提交手动解除预约限制并展示受控成功或错误信息
- 缺少 `violation.manual_blocks.write` 权限的管理员不得看到或成功提交手动预约限制开启/解除动作
- 违约记录页面的手动限制表单只调用 `violation` 公开 service/API，不直接写业务表
- 违约记录页面参数错误时返回统一 HTML 错误，不裸露 JSON
- 通知页面可展示和筛选通知日志，不提供预约前提醒、未签到提醒或超时未签到释放通知的手动触发按钮
- 无权限访问页面被拒绝或重定向
...
- 管理端可查询预约记录与统计结果
- 管理端可完成验收所需的当前动态签到码状态、签到记录和通知日志核对；后台自动任务不得依赖管理端按钮推进
- 管理端可在违约记录页面查看指定用户违约次数统计，并手动开启或解除该用户预约限制
- 已实现业务模块具备对应后台页面
- 表单提交与服务端返回一致
- 关键管理端页面测试通过
