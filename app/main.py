...
def _sanitize_validation_errors(exc: RequestValidationError) -> list[dict[str, object]]:
    ...
    for error in exc.errors():
        sanitized.append(
            {
                "loc": _make_json_safe(error.get("loc")),
                "type": _make_json_safe(error.get("type")),
                "msg": _make_json_safe(error.get("msg")),
            }
        )
    return sanitized
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def handle_validation_error(_: Request, exc: RequestValidationError) -> JSONResponse:
        return JSONResponse(
            status_code=422,
            content=_error_payload("validation_error", "Request validation failed.", _sanitize_validation_errors(exc)),
        )
