...
class UserRepository:
    ...
    def get_by_student_no(self, student_no: str, *, load_relationships: bool = False) -> User | None:
        stmt = select(User).where(User.student_no == student_no)
        if load_relationships:
            stmt = self._with_identity_graph(stmt)
        return self.session.scalar(stmt)

    def get_by_email(self, email: str, *, load_relationships: bool = False) -> User | None:
